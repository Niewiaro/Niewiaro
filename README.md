# Python, Full-Stack & Embedded Developer

I am a software engineer specializing in Python automation, modern web applications (Vue/Nuxt), and embedded systems (ESP32, C/C++). With an academic background in Industrial Automation & Robotics and Applied Computer Science, I bridge the gap between hardware and software, delivering end-to-end IoT solutions, scalable full-stack applications, and robust hardware prototypes.

<div align="center">
  <img src="https://komarev.com/ghpvc/?username=niewiaro&label=Profile%20views&color=0e75b6&style=flat" alt="profile views counter" />
</div>

---

## 👤 About Me

- 💻 **Current Role:** Full-Stack & Python Automation Developer at **🔷 EuroCert 🔷**
- ⏪ **Previous Experience:** Requirements Manager / Full-Stack & Python Automation Developer at **🟠 Aptiv 🟠**
- 🎓 **MSc in Applied Computer Science** (in progress) — Faculty of Mechanical Engineering, Cracow University of Technology
- 🎓 **BSc in Industrial Automation & Robotics** — Faculty of Mechanical Engineering, AGH University of Krakow
- ⚡ **Fun fact:** Big fan of sailing ⛵

> [!IMPORTANT]
> Big shoutout to my buddy and mentor,  
> **[@misiekroz](https://www.github.com/misiekroz)** — thanks for diving into embedded work together and making this coding journey awesome!

---

## 🏆 Hackathons & Hardware Competitions

I am a proud member and programmer of the **"Czterej Pancerni i Piec"** hackathon team. We specialize in embedded systems, IoT, and hardware challenges, and have secured several national-level victories across Poland.

- 🥉 **3rd Place — 11th BEST Hacking League** (Hardware Category)
  - **Date & Location:** Nov 29-30, 2025, Warsaw | **Organizer:** BEST Warsaw
  - **Theme:** Eco-innovations
  - **Project:** Distributed IoT chimney caps system for monitoring combustion products.
  - 🔗 [Check out the repo](https://github.com/Niewiaro/BHL-2025-Hardware)

- 🥇 **1st Place — Mechaton AGH 2025** (Robotics Category)
  - **Date & Location:** Nov 22, 2025, Cracow | **Organizer:** WRSS WIMIR AGH
  - **Project:** Programming an ASTOR industrial robot.
  - 🔗 [Check out the repo](https://github.com/Niewiaro/MECHATON-AGH)

- 🥇 **1st Place — RCDC 2025** (Remote Car Development Category)
  - **Date & Location:** Apr 26, 2025, Cracow | **Organizer:** BEST AGH Kraków
  - **Theme:** Built a remote-controlled car from provided raw components and competed against other teams on a dedicated race track.
  - 🔗 [Check out the repo](https://github.com/Niewiaro/MECHATON-AGH)

---

## 🛠️ Tech Stack

**Focus:** Python (Backend & Automation), Vue.js/Nuxt (Frontend), C/C++ & ESP32 (Embedded).

<div align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=py,ts,js,vue,nuxtjs,pinia,html,css,bootstrap,flutter,django,fastapi,flask,nodejs,postgres,sqlite,redis,rabbitmq,c,cpp,cs,arduino,raspberrypi,docker,cloudflare,linux,windows,git,github,gitlab,vscode,visualstudio,pycharm,androidstudio,matlab,md&perline=12" alt="Tech Stack" />
  </a>
</div>

---

> [!CAUTION]
> Beware of snakes!

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/Niewiaro/Niewiaro/output/github-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/Niewiaro/Niewiaro/output/github-snake.svg" />
  <img alt="github-snake" src="https://raw.githubusercontent.com/Niewiaro/Niewiaro/output/github-snake.svg" />
</picture>
